...
export class EngineModel {
  ...
  private buildYmlFunction(
    ...
    if (sourceType) {
      ...
      ymlAbstractFunction.detail = `[${sourceType}].${
        ymlAbstractFunction.label
      }`;
    } else {
  ...
  private enrichYmlClass(yclass: YmlClass): void {
    ...
    if (yclass.extends.length > 0) {
      yclass.detail = `Class: ${yclass.label} extends ${yclass.extends}`;
    } else {
